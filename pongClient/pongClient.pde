...
void draw(){
    ...
    if(controlling == 0){fill(100);}else{fill(255);}
    rect(50,yS[0],10,150);

    if(controlling == 1){fill(100);}else{fill(255);}
...
void gameLogic(){
  ...
  if(_client.available() > 0){
      ...
      if(controlling == 0){yS[1] = int(data[2]) + int(data[3]);} else {yS[0] = int(data[0]) + int(data[1]);}
   ...
   if(keyPressed){
      switch(key){
         case('w'):
         if(yS[controlling] > 5){yS[controlling] -= 10;}
         sendMove();
         break;
         case('s'):
         if(yS[controlling] + 150 < height){yS[controlling] += 10;}
         sendMove();
         break;
      }
   }
